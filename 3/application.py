...
def broker(options):
    ...
    Run broker
    '''
    api = middleware.Api(options.address, options.port)
    api.broker()
...
def publisher(options):
    ...
    while api.running():
        for topic_number in range(options.topics):
            api.publish('{}-topic'.format(topic_number), options.ownership, '{} {} {} {}'.format(address, sequence, random.random(), time.time_ns()), options.history)
            sequence += 1
...
def subscriber(options):
    ...
    while api.running():
        for topic_number in range(options.topics):
            data = api.notify('{}-topic'.format(topic_number), options.history)
            if data:
...
if __name__ == '__main__':
    ...
    publisher_parser.add_argument('-n', '--history', type=int, default=1, required=False, help='number of historical samples to track')
    publisher_parser.add_argument('-o', '--ownership', type=int, default=1, required=False, help='ownership strength')
    ...
    subscriber_parser.add_argument('-n', '--history', type=int, default=1, required=False, help='number of historical samples to request')
